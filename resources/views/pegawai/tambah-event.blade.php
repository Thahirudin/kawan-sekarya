...
@section('content')
    <section class="p-4 bg-white rounded-xl lg:p-5 mb-5 shadow-lg">
        <form action="{{ route('pegawai.store-event') }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('POST')
            <!-- Global Error Alert -->
            @if (session('error'))
            ...
            <div class="mb-5">
                ...
                <textarea
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                    id="deskripsi_singkat" name="deskripsi_singkat" oninput="updateSeoMeta()">{{ old('deskripsi_singkat') }}</textarea>
