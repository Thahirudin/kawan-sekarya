...
@section('content')
    <section class="p-4 bg-white rounded-xl lg:p-5 mb-5 shadow-lg">
        ...
        <div class="">
            <form action="{{ route('admin.update-pegawai', ['id' => Auth::guard('pegawai')->user()->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div>
                    <!-- Profile Header -->
                    <div class="lg:flex items-center gap-5 mb-8">
                        <img id="profileImage" src="{{ asset(Auth::guard('pegawai')->user()->gambar) }}"
                            class="w-36 h-36 bg-cover text-center mx-auto rounded-full" alt="">
                        <input type="file" class="bg-gray-200 w-full rounded-md text-gray-700"
